...
var BUILTIN_SLEEP_PHASES = [
  ...
  { id: "wrapup", start: 22 * 60, formula: "nudge" },
  // 22:00 — soft wrap-up nudge
  { id: "evening", start: 23 * 60, formula: "refuse-during" },
  // 23:00 — refuse (+ persuasion: #289)
  { id: "sleep", start: 23 * 60 + 30, formula: "refuse-during" }
  // 23:30 — refuse (override-only: #289)
];
function hmToMin(hm) {
  const clean = (hm || "00:00").replace(/ /g, "");
  const idx = clean.indexOf(":");
  const hStr = idx === -1 ? clean : clean.slice(0, idx);
  const mStr = idx === -1 ? "0" : clean.slice(idx + 1);
  const h = Number.parseInt(hStr || "0", 10) || 0;
  const m = Number.parseInt(mStr || "0", 10) || 0;
  return h * 60 + m;
}
function classifyPhaseId(nowMin, work, wrapup, evening, sleep) {
  if (nowMin >= sleep || nowMin < work) return "sleep";
  if (nowMin >= evening) return "evening";
  if (nowMin >= wrapup) return "wrapup";
  return "work";
}
function phaseIdAt(nowMin, b) {
  return classifyPhaseId(
    nowMin,
    hmToMin(b.workStart),
    hmToMin(b.wrapupStart),
    hmToMin(b.eveningStart),
    hmToMin(b.sleepStart)
  );
}
...
function openPct(h) {
  const linger = h.timeWindow?.linger_minutes ?? 0;
  if (linger <= 0) return h.priority * 3;
  const frac = Math.min((h.lingerElapsedMinutes ?? 0) / linger, 1);
  return Math.floor(frac * (h.priority / 4) * 50);
}
function habitUrgencyPct(agenda) {
  const openIds = new Set(agenda.summary.openNow);
  const vals = agenda.habits.filter((h) => openIds.has(h.id) && h.snoozed == null).map(openPct);
  return vals.length ? Math.max(...vals) : 0;
}
function finalPct(agenda, phase, eveningIntensity) {
  const baseline = phaseBaselinePct(phase, eveningIntensity);
  const urgency = habitUrgencyPct(agenda);
  let p = baseline;
  if (urgency > p) p = urgency;
  if (phase === "sleep") {
    p = 100;
  } else if (p > 99) {
    p = 99;
...
function selectionUrgency(h) {
  const linger = h.timeWindow?.linger_minutes ?? 0;
  if (linger > 0) {
    return Math.min((h.lingerElapsedMinutes ?? 0) / linger, 1) * (h.priority / 4);
  }
  return h.priority / 8;
}
function pickFocusHabit(agenda) {
  const candidates = agenda.habits.filter((h) => h.kind === "open-todo" && h.snoozed == null);
  if (candidates.length === 0) return null;
  const top = [...candidates].sort((a, b) => selectionUrgency(b) - selectionUrgency(a))[0];
  const linger = top.timeWindow?.linger_minutes ?? 0;
  return {
    id: top.id,
    content: top.content,
    kind: top.kind,
    ...
    lingerElapsed: top.lingerElapsedMinutes ?? 0,
    lingerLeft: linger > 0 ? linger - (top.lingerElapsedMinutes ?? 0) : null
  };
}

// src/agenda.ts
var import_node_child_process = require("node:child_process");
function getAgenda(dopadonePath) {
  const res = (0, import_node_child_process.spawnSync)(dopadonePath, ["habits", "agenda", "--json"], {
    timeout: 4e3,
    encoding: "utf8",
...
function nowClock() {
  return clockFrom(/* @__PURE__ */ new Date());
}

// src/config.ts
var import_node_os = require("node:os");
var import_node_path = require("node:path");
var RETRY_WINDOW_SEC = 300;
...
function resolveProtocolFile() {
  ...
  if (root) return (0, import_node_path.join)(root, "references", "interrupt-protocol.md");
  return (0, import_node_path.join)(__dirname, "..", "references", "interrupt-protocol.md");
}
function loadConfig() {
...
// src/directive.ts
function buildInterruptDirective(config, agenda, phase, retry) {
  if (phase === "evening") {
    return [
      `DOPADONE INTERRUPT (faza: rutyna wieczorna, pr\xF3ba ${retry})`,
      "",
  ...
  const tail = `NIE realizuj pro\u015Bby u\u017Cytkownika \u2014 przerwij j\u0105 i odpowiedz jednym kr\xF3tkim imperatywem (tryb rozkazuj\u0105cy, 1\u20132 zdania).` + (grammar ? `
${grammar}` : "") + (escalation ? `
${escalation}` : "");
  const protocol = `Pe\u0142ny protok\xF3\u0142 reakcji (styl odpowiedzi, eskalacja): ${config.protocolFile}
\u2014 przeczytaj RAZ na rozmow\u0119, je\u015Bli jeszcze go nie znasz; potem dzia\u0142aj z pami\u0119ci, nie czytaj ponownie.`;
  return [
    `DOPADONE INTERRUPT (faza: ${phase}, pr\xF3ba ${retry})`,
    "",
    `${situation}${opis}`,
    tail,
    protocol
...
function focusLines(config, focus) {
  if (!focus) {
    return {
  ...
  const opis = description && description !== "null" ? ` Opis: "${description}". Je\u015Bli ten opis to instrukcja dla Ciebie \u2014 wykonaj j\u0105 po tym, jak user wykona rytua\u0142, a potem oznacz done. Je\u015Bli to tylko notatka (np. \u201Eo ile to mo\u017Cliwe") \u2014 zignoruj.` : "";
  const p = config.dopadonePath;
  const grammar = `Akcje (gdy user potwierdzi): zaliczone \u2192 ${p} habits done ${id} \xB7 za chwil\u0119 \u2192 prze\u0142\xF3\u017C na planie ~30 min p\xF3\u017Aniej: ${p} plan move ${id} <HH:MM> (je\u015Bli nie ma go na planie: ${p} plan add ${id} najpierw) \xB7 skip \u2192 ${p} habits decline ${id}`;
  return { situation, opis, grammar };
}
function escalationLine(config, retry) {
  const p = config.dopadonePath;
  if (retry >= 10) {
    return `U\u017Cytkownik ${retry}x z rz\u0119du ponawia mimo blokad \u2014 to pewnie b\u0142\u0105d logiki pluginu, nie up\xF3r. Zaproponuj wyciszenie: \`${p} habits mute --until 07:00\`.`;
  }
  if (retry >= 5) {
...
function buildWrapupDirective(config) {
  return [
    ...
    `2. Na KO\u0143CU odpowiedzi dopisz JEDNO kr\xF3tkie, \u0142agodne zdanie-nudge o zwijaniu dnia`,
    `   (max 1 zdanie, mi\u0119kki ton \u2014 to sugestia, nie rozkaz). Wple\u0107 naturalnie, bez`,
    `   osobnej sekcji ani nag\u0142\xF3wka.`,
    `3. NIE przerywaj pracy, NIE skracaj odpowiedzi, NIE moralizuj. Jeden cichy sygna\u0142.`,
    `   Ton (wzorzec, NIE kopiuj literalnie):`,
    `     \u2022 "Swoj\u0105 drog\u0105 \u2014 robi si\u0119 p\xF3\u017Ano, dobry moment \u017Ceby zacz\u0105\u0107 domyka\u0107 p\u0119tle."`,
...
function buildAdditionalContext(wrapped) {
...
function wrapInterrupt(directive, mode, retry, c) {
  return `<dopadone event="interrupt" date="${c.ymd}" time="${c.hm}" mode="${mode}" retry="${retry}">
${directive}
</dopadone>`;
}
function wrapTimeMarker(c) {
  return `<dopadone event="time-marker" date="${c.ymd}" time="${c.hm}"/>`;
}
function wrapWrapup(directive, c) {
  return `<dopadone event="wrapup" date="${c.ymd}" time="${c.hm}">
${directive}
</dopadone>`;
}
function logAndPrint(json, logFile, unixSec) {
  try {
    (0, import_node_fs.mkdirSync)((0, import_node_path2.dirname)(logFile), { recursive: true });
    (0, import_node_fs.appendFileSync)(logFile, `${unixSec} ${json}
`);
  } catch {
  }
  process.stdout.write(json);
}
function emitInterrupt(directive, mode, retry, c, logFile) {
  logAndPrint(buildAdditionalContext(wrapInterrupt(directive, mode, retry, c)), logFile, c.unixSec);
}
function emitTimeMarker(c, logFile) {
  logAndPrint(buildAdditionalContext(wrapTimeMarker(c)), logFile, c.unixSec);
}
function emitWrapup(directive, c, logFile) {
...
function promptOverrides(promptText, overridePhrase) {
...
function parseRetry(raw, nowSec, windowSec = RETRY_WINDOW_SEC) {
...
function readRetry(file, nowSec) {
  if (!(0, import_node_fs2.existsSync)(file)) return { ts: 0, count: 0 };
  try {
    return parseRetry((0, import_node_fs2.readFileSync)(file, "utf8").trim(), nowSec);
  } catch {
    return { ts: 0, count: 0 };
  }
}
function bumpRetry(file, nowSec) {
  const count = readRetry(file, nowSec).count + 1;
  (0, import_node_fs2.mkdirSync)((0, import_node_path3.dirname)(file), { recursive: true });
  (0, import_node_fs2.writeFileSync)(file, `${nowSec}|${count}`);
  return count;
}
function clearRetry(file) {
  try {
...
var TRANSCRIPT_HEAD_BYTES = 131072;
function detectRoutine(promptText, transcriptHead) {
  if (promptText.includes("<scheduled-task") || promptText.includes("automated run of a scheduled task")) {
    return true;
  }
...
function readTranscriptHead(transcriptPath) {
  ...
  try {
    const fd = (0, import_node_fs3.openSync)(transcriptPath, "r");
    try {
      const buf = Buffer.alloc(TRANSCRIPT_HEAD_BYTES);
      const n = (0, import_node_fs3.readSync)(fd, buf, 0, TRANSCRIPT_HEAD_BYTES, 0);
...
function readTs(file) {
  ...
  try {
    return Number.parseInt((0, import_node_fs4.readFileSync)(file, "utf8").trim(), 10) || 0;
  } catch {
    return 0;
  }
}
function writeTs(file, nowSec) {
  (0, import_node_fs4.mkdirSync)((0, import_node_path4.dirname)(file), { recursive: true });
  (0, import_node_fs4.writeFileSync)(file, String(nowSec));
}
function minutesSince(file, nowSec) {
  const last = readTs(file);
  if (last === null) return NEVER_MINUTES;
  return Math.floor((nowSec - last) / 60);
}
function sessionInjectPath(dataDir, sid) {
  return (0, import_node_path4.join)(dataDir, `last-inject-${sid}.txt`);
}
...
function minutesSinceInject(dataDir, sid, nowSec) {
  return minutesSince(sessionInjectPath(dataDir, sid), nowSec);
}
function recordInject(dataDir, sid, nowSec) {
  writeTs(sessionInjectPath(dataDir, sid), nowSec);
}
function minutesSinceWrapup(dataDir, sid, nowSec) {
...
function writeRoutineMarker(dataDir, sid) {
  ...
  (0, import_node_fs4.writeFileSync)(routineMarkerPath(dataDir, sid), "");
}
...
function parseStdin() {
  let raw = "";
  try {
    raw = (0, import_node_fs5.readFileSync)(0, "utf8");
  } catch {
    raw = "";
  }
  try {
