...
// src/notification.ts
function computeNotification(c, config) {
  let title = "";
  let body = "";
  if (c.min < 15) {
    if (c.hour === 8) {
      ...
      title = "Obiad";
      body = "Okno 13:00-15:00. Czas zam\xF3wi\u0107 / zje\u015B\u0107.";
    } else if (c.hour === 18) {
      title = "Kolacja";
      body = "Okno 18:00-20:00. Czas zje\u015B\u0107.";
    }
  }
  const fireNear = (boundary, t, b) => {
  ...
  if ((c.minOfDay - warnMin + 1440) % 1440 < 15) {
    title = "Sen za 10 min";
    body = `${config.sleepStart} lights-out. Domykaj dzie\u0144, do \u0142\xF3\u017Cka.`;
  }
  if (!title) return null;
  return { title, body };
}
function notifDedupKey(c, title) {
...
function main() {
  ...
  if (!notif) return;
  const stateFile = (0, import_node_path2.join)(config.dataDir, "health-rhythm-notif.txt");
  const nowKey = notifDedupKey(c, notif.title);
  let lastKey = "";
  if ((0, import_node_fs.existsSync)(stateFile)) {
    try {
      lastKey = (0, import_node_fs.readFileSync)(stateFile, "utf8").trim();
    } catch {
      lastKey = "";
    }
  }
  ...
  (0, import_node_child_process2.spawnSync)(
    ...
    [
      "-e",
      `display notification "${notif.body}" with title "\u{1F957} ${notif.title}" sound name "Glass"`
    ],
    { stdio: "ignore" }
  );
  (0, import_node_fs.mkdirSync)((0, import_node_path2.dirname)(stateFile), { recursive: true });
  (0, import_node_fs.writeFileSync)(stateFile, nowKey);
}
